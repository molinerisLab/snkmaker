...
HERE IS THE HISTORY:`;
    ...
    async process_TEST(request, context, stream, token) {
        const prompt = request.prompt;
        const llm = this.viewModel.llm;
        console.log("Prompt: ", prompt);
        const response = await llm.run_query(prompt);
        console.log("Response: ", response);
        let markdownCommandString = new vscode.MarkdownString(response);
    ...
    async process(request, context, stream, token) {
        const messages = [
            ...
            vscode.LanguageModelChatMessage.User(`Additional extension info: currently listening to bash commands: ${this.viewModel.isListening}. Copilot active: ${this.viewModel.isCopilotActive()}  Currently changing model: ${this.viewModel.isChangingModel}. Models available: ${this.viewModel.llm.models.map((m) => m.get_name())}. Active model: ${this.viewModel.llm.models[this.viewModel.llm.current_model]?.get_name() || 'none'} - Logging status: ${SnkmakerLogger_1.SnkmakerLogger.logger_status()}`)
        ];
        ...
        for await (const fragment of chatResponse.text) {
            ...
            let markdownCommandString = new vscode.MarkdownString(f);
        ...
        SnkmakerLogger_1.SnkmakerLogger.instance()?.log(`
