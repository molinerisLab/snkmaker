...
-For reproducibility it is important to keep track of the conda environments used. Snakemaker tracks them, outputs yaml file describing the envs and add a 'conda' directive to the produced rules. It can be disabled in the settings related to Snakemake best practices.
-A setting named GenerateConfig determines if the model looks for repeating patterns or hardcoded values to be moved to a config.yaml file. It can slow down rule generation. It can be disabled.
...
HERE IS THE HISTORY:`;
    ...
    private getBasePrompt(is_chat_panel: boolean, has_open_notebbok: boolean = false): vscode.LanguageModelChatMessage[] {
        ...
        return [
            ...
            vscode.LanguageModelChatMessage.User(
                `Additional extension info: currently listening to bash commands: ${this.viewModel.isListening}.`+
                ` Copilot active: ${this.viewModel.isCopilotActive()}  Currently changing model: ${this.viewModel.isChangingModel}.`+
                ` Models available: ${this.viewModel.llm.models.map((m) => m.getName())}.`+
                ` Active model: ${this.viewModel.llm.models[this.viewModel.llm.current_model]?.getName()||'none'} -`+
                ` Logging status: ${SnkmakerLogger.loggerStatus()} - Current rule format: ${rule_format} -`+
                ` Snakemake rules contains Log directive: ${containsLogField} - `+
                `Snakemake rules uses generic filenames and wildcards: ${preferGenericRules} - `+
                `automatic validation of snakemake rules: ${snakemakeValidation} - `+
                `Keep history between sessions: ${mustStash} - Current file included in prompt: ${ExtensionSettings.instance.getIncludeCurrentFileIntoPrompt()} - `+
                `CommentEveryRule: ${ExtensionSettings.instance.getCommentEveryRule()}`+
                `Track conda env: ${ExtensionSettings.instance.getAddCondaDirective()} - `+
                `Generate config: ${ExtensionSettings.instance.getGenerateConfig()} - `
            )
        ];
    }
