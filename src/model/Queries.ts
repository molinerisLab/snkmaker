...
Please write "YES" if it's worth making into a rule, "NO" if it's a one-time command. DO NOT, EVER output other things, only YES or NO.`;
    ...
    async get_snakemake_rule(bashCommand: BashCommand){
        ...
        var query = `Convert this bash command into a ${rule_format} rule:
${command}
...
Please output only the rule. What you output goes entirely in the ${rule_format} file, so Do not output other things. Example of good output: "<RULE>". Examples of bad output: "Here is the rule <RULE>" or "<RULE> is the rule".`;
    ...
    async get_all_rules(commands: BashCommand[]){
        const rule_format = this.get_rule_format();
        var extra_prompt = "";
        if (rule_format==="Snakemake"){
            extra_prompt = `Please write the Snakemake rules following these guidelines:
1- Prefer the usage of generic names for input-outputs using wildcards, when possible. For example input: "data.csv", output: "transformed_data.csv" could be written as input: "{file}.csv", output: "transformed_{file}.csv".
2- Multiple commands that execute the same operation on different files can be merged in a single rule using generic inputs/outputs and wildcards. Following the example above, another rule could be input: "GENES.csv", output: "transformed_GENES.csv" could be merged with the previous rule.
3- If using generic filenames, add a "all" rule that generates the files generated by the user.`;
            if (this.add_log_to_rule()){
                extra_prompt += "\n4- Add a log field to each rule with the name of the log file. For example, log: 'logs/{rule}.log'. If the rule contains wildcards, they must be part of the log file name or Snakemake will throw an error. Log fields must be added before shell fields.";
            }
        }
        ...
        ${formatted.join("\n")}
Please do not remove the new-lines chosen by the user. You might add new-lines for readability but only if necessary. ${extra_prompt}
